...
def generate_all(folder_path, mask_folder_path, output_folder_path):
    distance_map_output_folder = output_folder_path+"/distance_maps"
    point_cloud_output_folder = output_folder_path+"/point_cloud"
    voronoi_reconstruction_folder = output_folder_path+"/voronoi_reconstruction"
    prep_folder(distance_map_output_folder)
    prep_folder(point_cloud_output_folder)
    prep_folder(voronoi_reconstruction_folder)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generates the Voronoi representation of a stack of images using a mask")
    parser.add_argument("data_folder", type=Path, help="Path to the data folder (must already exist).")
    parser.add_argument("mask_folder", type=Path, help="Path to the mask folder (must already exist).")
    parser.add_argument("output_folder", type=Path, help="Path to the output folder (will be cleared or created).")
    args = parser.parse_args()
    validate_folder(args.data_folder)
    validate_folder(args.mask_folder)

    generate_all(args.data_folder, args.mask_folder, args.output_folder)
